...
The retained v0.9.1 agent guidance establishes a minimal entity-appropriate basis before focused portrayal when the player singles out an individual or the entity directly engages the PC, and also when an established perception, order, due process or authorized initiative makes the basis material. Player attention is not the fictional activation switch. The PC's interpretation cannot establish private nature, and late deepening cannot invent an earlier hidden cause merely to explain or contradict an observed presentation. See [Independent agent state](OS/AGENT_STATE.md).
...
3. Give the GM the v0.9.2 kit as the update source and say: "Open ADMIN/UPGRADE_V08.md from the new kit and prepare this campaign's upgrade in the separate copy."
...
Valid v0.7.3, v0.8.0, v0.8.1, v8.1.1, v0.9.0 and v0.9.1 records retain their formats and save identities. The upgrade consolidates general operating guidance; it does not resample NPCs, rewrite historical motives, replace an accepted relationship, or require retrospective completion of personality fields. Only a separately accepted campaign change revises its agreement, generation method or autosave permission.
