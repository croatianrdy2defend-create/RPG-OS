...
The compact LAW rule governs ordinary portrayal and relevant unresolved outcomes. Direct attention selects the individual layer: when the player singles out a person for attention or interaction, or that person directly engages the PC, retrieve or establish the minimal current individual baseline before responding or resolving a reaction. A bare “I approach her” suffices without an inferred PC purpose or a question about motive. Background crowds remain background; a directly engaged clerk can have a tiny baseline. [OS/AGENT_STATE.md](OS/AGENT_STATE.md) supplies establishment, reaction, retention and later fallback guidance without an unconditional startup read or separate NPC database.

The current baseline uses a few relevant individual facts, such as present activity or purpose, orientation, manner and constraints. These are examples, not compulsory fields or a full personal profile. Reuse established source state, including private facts unknown to the PC. Deliberately unfixed properties follow their trigger; missing authority is a retrieval or repair problem. Eligible gaps use the accepted authoring or generation procedure before influencing resolution. A baseline is current state: actual events, information or completion of an activity can change it; repeated attention does not regenerate it or make a temporary mood permanent.

The NPC's perception is a separate layer. Use only observable PC facts and the NPC's actual prior knowledge. A mistaken inference can govern its belief without becoming the PC's true intent. Resolve its reaction from the baseline and applicable ENGINE procedure, without invented modifiers, duplicate resolution, rolling away fixed state or creating prior motives after the result. Direct attention need not be observable to the NPC. Later depth and durable promotion follow actual developments and continuity needs, not a cast-wide scan or an automatic file for every conversation.

The baseline does not predetermine every later outcome. Resolve a material open question through retained state, actual developments, applicable engine rules or supported judgment. When an eligible outcome remains too uncertain, use the accepted fallback oracle: frame a bounded yes/no question, set the mapping and obtain real random input. The supplied convention is one d6, 1–3 No and 4–6 Yes; another already accepted oracle keeps its method. A standing selection needs no per-roll permission. This fallback does not replace the initial current baseline or repeat an already resolved reaction. Sparse remaining detail is not a reason to default to inaction or invent a biography first. Retain the scoped result and stop at the first applicable interruption. The [worked fallback examples](MECHANICS.md#random-fallback-for-unresolved-outcomes) cover contact, promises and secrets. This is model-operated adjudication, not an independently running event processor.
