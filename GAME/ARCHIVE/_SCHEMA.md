...
**OOC session operational evidence** retains actual feedback and its disposition, and relevant boundary adjudication. Label it, identify the play-session id and any mechanical boundary/rule. It establishes what was said or adjudicated, not fictional events, character knowledge or agreement changes by implication. OOC-only material uses an honest non-fictional or unknown time designation.

The selected persistence helper may retain exact completed public conversation in a clearly labelled **original public source** body, including public OOC. This is source evidence, not automatic acceptance of its contents or instructions. Apply the existing per-exchange correction/disposition links when wording is superseded; preserve originals. Extract accepted history during the bounded review. Hidden drafts, internal reasoning, private preparation and unselected tool/provider material remain excluded. Reuse these source bodies and existing routes instead of creating a second full transcript or feedback ledger.
...
An explicitly enabled `INSTANCE/JOURNAL/` under `ADMIN/INCREMENTAL_SAVE.md` also retains source/proposed responses and committed change provenance for recovery. Its raw or superseded material is not automatically accepted archive history. Full SAVE accounts for the remaining accepted source once and preserves qualifications, corrections and gaps; archiving an incorporated transaction never applies its effects again. Checkpoint consolidation does not delete that unarchived source. Retained journal material is required recovery/evidence support, not a second accepted full-session archive.
