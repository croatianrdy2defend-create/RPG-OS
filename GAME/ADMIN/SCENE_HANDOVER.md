...
For the selected write-only log, use the full-save branch of step 3 before freezing the source. Retain its compiled/source cursors and log. The receiver cannot append to frozen source; return/import or cancellation resolves that boundary before source play resumes. An actual legacy rebinding is ADMIN work, never permission to restart its former turn protocol.
...
3. Establish the exact transfer present through CLOSE_CONTRACT’s selected executor. For write-only-log mode, compile outstanding notes and preserve remaining available source with full SAVE; otherwise CHECKPOINT may preserve the present while the package retains conversation. Reuse a completed save that already covers this exact state and required source—do not checkpoint it again. Exclude retired tests/unrelated conversations, preserve pending choices and pass no fictional time.
4. Reuse the original user-visible source already preserved for this boundary and collect only missing available coverage needed for the transfer. Retain roles, ordered turn identities, dialogue/actions and correction status; label OOC and gaps. Put meaningful tool-established facts/rolls in the briefing with source references. Exclude provider instructions, private reasoning, credentials and unrelated activity. A summary stays labelled; completeness requires actual coverage through the last included turn.
...
`source_gaps` is an empty list only with complete coverage; partial or summary coverage must name its gaps. All IDs bind to the current campaign, selected save and agreement. Preserve that save’s actual archived evidence boundary. The source map cites exact current/unchanged source routes and conversation turn IDs; summaries, private beliefs and unprepared answers retain their qualifications.
