...
CLOSE_CONTRACT owns review selection and publication. Perform the source-first comparison above once for the selected boundary, independently of proposed edits. Reuse exact captures and unchanged relevant reviewed facts with their stated limits; widen scope only for a dependency or conflict. A completed review can still find inconsistency or missing evidence. Do not substitute receipt production or repeated same-context rereading for semantic comparison.

At accepted periodic maintenance, a bounded historical spot-check may inspect older obligations no longer visible in current open matters; it is not a whole-archive review. If source, reviewer or budget is unavailable, preserve progress and report incomplete/deferred scope. Reconcile overlapping conversation/export by actual identities and spans, never wording alone. Private factual determinations retain their access limits; unrecorded ones cannot be reconstructed.
...
Use CLOSE_CONTRACT’s selected executor and review ordering. A compatible helper preview exposes the exact proposed affected records; compare them with independently selected source/prior authorities through this existing audit workflow. After review, publish those bytes once and verify the receipt binding. A changed proposal cannot reuse an incompatible approval. An unavailable review is reported incomplete, while known corrupt proposals remain unpublished and new progress is preserved separately. Post-publication repair follows CORRECT.
