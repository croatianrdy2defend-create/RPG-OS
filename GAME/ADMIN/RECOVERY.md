...
Compare the restored write set to its recorded preimages and verify the prior save/agreement, routes, and directory structure agree: protected pre-existing directories remain, operation-created residue is resolved, and no unexpected content was discarded.

For a bind with pending generated determinations recorded under `ADMIN/LOAD.md`, physical restoration alone does not cancel the accepted setup. Retain `status: started`, note that the files were restored but the bind remains pending, and keep ACTIVE pointing to that operation and its exact retained result paths. Resume the already authorized bind with those inputs when its target is available; a missing result remains a disclosed gap. Clear this pending bind only after verified completion or explicit operator cancellation/replacement of that setup, recording the disposition of its determinations. This preserves the retry route without scanning recovery history or inventing a new state store.

For a completed restoration, including a bind whose pending setup was explicitly cancelled/replaced, record `status: restored` and the result and remove the active marker. If an interrupted completion already marked the operation complete but left the marker, verify that completion before removing it. Do not merely trust the status word.
