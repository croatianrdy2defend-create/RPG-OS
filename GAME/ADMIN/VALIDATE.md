# VALIDATE — v0.9.0 experimental structural inspection
...
- Required kit files, including RECOVERY, CORRECT, UPGRADE_V07, UPGRADE_V08, the cold AGENT_STATE protocol and runtime source/evidence helpers; portable path containment, symlink routes, case collisions, UTF-8 and unfinished fixed-path candidates. Required presence is a playable-kit check, not a normal startup read requirement. Explanatory documents, test protocols, developer suites and historical release notes are optional repository material and are not required in a campaign workspace.
...
It does not execute a GM, test fresh-chat behavior, or observe the host's read/write isolation. AUDIT self-report does not establish those results. Use the separate behavioral and maintenance fixtures in https://github.com/croatianrdy2defend-create/RPG-OS/blob/main/DOCS/ADMIN/TESTS.md.

An agent-state file's presence does not establish independent NPC decisions, unbiased generation, genuine causal change, privacy or enjoyable play. Those are experimental claims to observe in actual campaigns using https://github.com/croatianrdy2defend-create/RPG-OS/blob/main/DOCS/ADMIN/PLAYTEST_V08.md. The handover checker protects recorded source bytes and package identity; it cannot prove that the receiving model interpreted or portrayed them faithfully.
...
The distribution includes `TOOLS/read_source.py`, `TOOLS/search_index.py`, `TOOLS/evidence.py`. Their focused regression suites live separately in the repository DEV directory and are not shipped in the playable kit. Runtime helper presence is part of kit validation; running normal VALIDATE does not execute an evidence audit or certify a cache. `ADMIN/SOURCE_ACCESS.md` and `ADMIN/EVIDENCE_AUDIT.md` document those separate commands.
