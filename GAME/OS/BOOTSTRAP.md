...
2. Read `OS/LAW.md` and `INSTANCE/CAMPAIGN_CONTRACT.md`.
   ...
   Read `INSTANCE/CURRENT_SAVE.md`. With accepted `Incremental recording: write-only-log`, read `ADMIN/PLAY_PERSISTENCE.md` and use `python -X utf8 -B TOOLS/play_log.py --root . read` once for uncompiled notes. Consider available conversation and compiled state together; a note proves no delivery. Before first PLAY, complete LOAD’s selected one-time initialization; on a changed task use INCREMENTAL_SAVE’s ADMIN source-binding transition. Reconcile material contradictions or outstanding legacy journal work through ADMIN without discarding progress. Ongoing PLAY uses context: no persistence resume/prepare/confirmation or repeated log read.
