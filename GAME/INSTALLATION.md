# Installation and portability — v0.9.8 experimental

## Standard write-only log for new campaigns

New v0.9.8 campaigns select the [write-only play log](ADMIN/PLAY_PERSISTENCE.md) by default unless the operator declines or customizes it during setup. Its normal-turn requirement is a compatible Python interpreter and permission to append a short factual note through `TOOLS/play_log.py`; one GM continues the campaign. No per-turn source inspection, receipt, hash, owner patch, callback or background worker is required. Original-source access is used at ADMIN full SAVE for actual available dialogue and review, with gaps disclosed.

Preserve a complete backup for adoption and keep original dialogue, working log, compiled state and retained legacy/recovery material. Fresh boot reads compiled records and uncompiled notes once. Compile and full-save before an incompatible switch; an older reader cannot silently ignore newer working notes. An export-only host must disclose its lack of an actual append operation.

## A new campaign

Download the [latest repository ZIP](https://github.com/croatianrdy2defend-create/RPG-OS/archive/refs/heads/main.zip), extract it, and copy its **GAME** subfolder into a dedicated campaign folder. Give that folder to your LLM; use one active campaign per copy. Start from [Quick start](QUICKSTART.md). Current main includes the 13 September v0.9.7 saving fixes; earlier tagged downloads keep their original bytes. The kit has blank unbound templates and Freeform, with no campaign module, character or artwork. The shared encounter helper is an optional procedure, not another engine. [Older releases](https://github.com/croatianrdy2defend-create/RPG-OS/releases) remain available.
...
The agent-state procedure uses existing records for entity-appropriate nature/capabilities, control, information, applicable aims/drives/directives, circumstances and relationships where they exist. Setup can establish the small private opening basis under the accepted authoring scope. New state remains in working conversation until ADMIN compilation; the selected short log provides concise outcome notes without rewriting the full state each reply. A new v0.9.8 campaign proposes the short log by default; upgrading an existing campaign does not silently enable it or any invisible memory service.

New v0.9.8 campaigns also propose [announced checkpointing](ADMIN/AUTOSAVE.md) by default: a protected complete-present CHECKPOINT on a 20-completed-PLAY-reply cadence after advance notice. Existing campaigns keep their prior autosave choice unless explicitly recalibrated. No daemon or mandatory script is required. TOOLS/autosave.py is an optional read-only scheduling helper, not a saver. Context-pressure triggers require actual model-visible telemetry or an operator report. A visible UI meter does not prove the model can read it, and no fixed percentage guarantees enough capacity for a complete save.
...
Encounter generation and unresolved-outcome fallback are selected through the accepted engine and agreement. Existing facts, actual capacities and individual history constrain them; installation changes neither an existing method nor a diceless campaign. See [independent agents and optional methods](https://github.com/croatianrdy2defend-create/RPG-OS/blob/main/DOCS/MECHANICS.md#independent-people-and-other-agents) for their workings.
...
3. Give the GM the v0.9.8 kit as the update source and say: "Open ADMIN/UPGRADE_V08.md from the new kit and prepare this campaign's upgrade in the separate copy."
...
The receiver leaves source authorities frozen. Its new unsaved facts must be captured in the permitted return; the source cannot be used as a live receiver checkpoint. Unavailable chat, unrecorded model intentions and inaccessible references cannot be transferred. See [Real-campaign playtesting](https://github.com/croatianrdy2defend-create/RPG-OS/blob/main/DOCS/ADMIN/PLAYTEST_V08.md).
