...
Only an explicitly enabled agreement selects `ADMIN/INCREMENTAL_SAVE.md` and `TOOLS/persistence.py`. The default unbound kit has no journal. An enabled run has `INSTANCE/JOURNAL/HEAD.json` plus immutable `batches/` and `receipts/` material under the helper's versioned format. This is required recoverable state, not a cache or a second semantic owner. Current values still belong to the same INSTANCE paths; the effective reader applies their committed pending edits to the consolidated baseline. New routes/records must also be visible through that reader. Prepared responses are excluded until actual completed delivery is confirmed.

The helper owns two optional nonnegative integer fields in CURRENT_SAVE's existing metadata table: `journal_through` identifies the incorporated transaction boundary; `journal_archive_through` identifies the journal source boundary accounted for by the accepted archive operation. At first explicit initialization an absent cursor means zero, not evidence of an old journal's loss. CHECKPOINT advances incorporation while preserving the archive cursor and ordinary archive metadata. Full SAVE accounts for remaining accepted source without reapplying its state effects. These cursors add no new `commit_kind`, fictional event or review verdict. Outside the enabled experiment, existing absence remains compatible; an enabled store cannot be safely ignored by an older reader.

Helper-owned metadata also binds `journal_stream`, the incorporated `journal_head`/`journal_head_sha256`/`journal_head_receipt_sha256`, and archived `journal_archive_head`/`journal_archive_sha256`/`journal_archive_receipt_sha256` where applicable. These operational anchors verify the cursor's actual batch and delivery-receipt chain; they are not second fact owners or editable GM summaries. Preserve their exact helper-produced values with the corresponding save. A pending batch is also digest-bound by HEAD. An interrupted `PREPARE_INTENT.json` is recovered to its exact retained prepared target before delivery is separately confirmed. The helper validates consistency; a plain structural validator's tolerance of optional fields is not a journal-integrity verdict.

Bind the store to the actual campaign, agreement, conversation and base file identities; use an available verified original save-message source floor when adopting existing play. Expected hashes, predecessor checks and unique transaction identities reject detectable stale/duplicate application. Interrupted publication is reconciled through its retained helper operation; no manual cursor repair. Backups/exports include the journal, baseline and necessary source/recovery material. Disabling requires no pending work, complete consolidation/archive coverage and an explicit disabled agreement; the helper retains `DISABLED-<stream_id>.json` and all history while removing the active HEAD by atomic rename. Retained nonzero history is not automatically re-enabled by initialization. See the procedure for foreground completion limits, verified export and supported administrative operations.
...
- **Campaign promise:** accepted experience, scope, fit, exclusions, source/fidelity expectations, and the Play form clause. Campaign spirit fits here as compact prose connecting accepted experience priorities to practical GM decisions; use `ADMIN/NEW_GAME.md#campaign-spirit`. It adds no required field or clause, and existing agreements remain valid without that label or an invented preference. Knowingly accepted structural destinations or fixed outcomes may exist within their actual granted scope; do not present those as open resolution or override still-reserved PC decisions.
...
By default PLAY writes no campaign files; only explicitly enabled incremental persistence adds its narrow prepared/confirmed exchange operation. Save/bare CLOSE uses ADMIN/CLOSE_CONTRACT.md; an actual END SESSION request uses ADMIN/SESSION.md before that full save. CHECKPOINT uses CLOSE's reduced-evidence path. An enabled journal requires the helper's coordinated publication. All preserve complete current authorities, administrative continuity and useful existing preparation without independently adjudicating boundaries. REVIEW writes only optional notes. RECALIBRATE writes only an explicitly accepted prospective agreement. CORRECT follows ADMIN/CORRECT.md for narrow current and historical repair. Safety changes take effect in the conversation immediately; durable changes use the recovery procedure and matching save flag.

Independent-agent establishment, updating and deactivation add no write permission by themselves. Without enabled incremental recording, the active set changes in conversation with best-effort retention; the next authorized save synchronizes its complete current membership and values. With recording enabled, retain established changes through that procedure without generating missing state or a per-person checkpoint. Requested CHECKPOINT still preserves every accepted current change, active baseline, consequential retained fact and pending declaration, not just the encounter block. The receiving GM cannot checkpoint or append to frozen source authorities. Missing required retained state requires honest source/repair handling, not silent regeneration. Program adoption preserves existing saved facts; only subsequent authorized lifecycle transitions can expire eligible temporary state.

Before overwriting campaign owners, follow ADMIN/RECOVERY.md: verified preimages and planned new paths in RECOVERY/<operation-id>, plus RECOVERY/ACTIVE.md. Narrow incremental batch/head writes use the helper's documented protected operation; this exception grants no arbitrary file writes. Any present recovery marker blocks ordinary boot until recovery or completion is verified. CURRENT_SAVE is published last for bind/save, with an enabled journal's boundary coordinated by the helper; this is not multi-file atomicity. Preserve recovery materials; never claim automatic rollback. A failed review or agreement operation does not change unrelated state, but any pending recovery marker must be resolved before PLAY.
