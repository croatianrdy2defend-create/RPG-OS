...
Use [PLAYTEST_V08](PLAYTEST_V08.md) for actual campaign observations and [UPGRADE_V08](../../GAME/ADMIN/UPGRADE_V08.md) for a prospective, scoped upgrade of an existing campaign. The reference fixtures below remain available for investigating a reproduced problem; they are not a compulsory onboarding exercise or a claim that the experimental protocol has passed behavioral evaluation. Keep campaign evidence and personal playtest notes out of the public fresh-install archive.
...
Diagnostic, NOT RUN. These six optional manual cases use disposable original campaign copies; they do not certify model behavior or require a visible PLAY checklist. The operative OS requires five temporary Initial Encounter Baseline dimensions before individual participation, appraisal or response, even when the player has not declared a purpose. Required factual coverage does not require five persisted columns. The focused cases in `TEST_AGENT_STATE.md` U16–U26 cover entity-compatible state, group scale, single signed attraction, world bindings, focused portrayal, activation, attention gaps, expiry, retained consequences and save/resume ownership.

| Case | Expected behavior |
|---|---|
| Attention and initiative | Compare a perceived approach to an incidental stranger, visual-only observation of an unaware clerk, and an NPC initiating a look or contact with the PC. Establish the required current baseline before individual appraisal, engagement or reaction, while focused appearance alone retrieves only its required portrayal sources. Do not infer company, flirting, a request or other undeclared PC purpose, or require such a purpose before participation activates. Quiet observation adds no approach or NPC awareness. A crowd glance or ten soldiers merely spotting the PC does not initialize every person separately; three drink recipients whose separate decisions matter each require their own basis. |
| Current individual and knowledge | Account for all five temporary dimensions using supported current facts, governing-owner references or precise source/capacity/perception limitations; no personal-life inventory, enduring trait draw or invented completeness is required. Distinguish what the NPC can perceive or know about the PC from hidden PC values and OOC wishes. An unperceived PC produces no initial PC-directed appraisal. An ENGINE can use required actual values without making them NPC knowledge. Neither an assumed player goal nor a selected outcome supplies a retrospective motive. |
| Existing and protected state | Reuse an established person's active baseline. Separately supply an eligible unauthored detail, an inaccessible required active fact and a deliberately unfixed answer. Establish only the eligible scope; retrieve the missing source or preserve the protected trigger. Contrast genuine, completed deactivation of an inconsequential baseline: a later encounter may generate fresh temporary state within retained facts. Neither a sparse live record nor attention shifting permits a reset. |
| Baseline before resolution | Observe the baseline's establishment before any response/reaction draw or outcome it governs. Applicable ENGINE procedures and established constraints come first. Selected initial generation uses its actual bound mappings and real inputs only for eligible initial facts, including initial attraction where expressly accepted. Ordinary Tellus social reception remains grounded judgment, with no willingness/consent or substitute reception roll. The standing fallback's 1–3 No, 4–6 Yes convention applies only within its own eligible scope. Preserve each result and do not resolve the same question again under another label. |
| Development and reuse | Continue the active encounter across ordinary attention gaps: retain the five temporary dimensions and reuse any scoped resolved reaction. A meaningful development updates only affected current facts; concrete priorities can compete with an attractive or friendly impression. Later first perception may establish only the still-eligible PC-directed appraisal, without manufacturing an earlier cause. Genuine deactivation releases inconsequential temporary state while retaining operative consequences. Do not create an all-cast scan, timer or inactive registry. |
| Save and fresh chat | Verify a requested full save and resume the same active encounter in a fresh chat. A compact NOW block preserves temporary active values and useful generation references; PEOPLE and existing system owners retain durable facts and consequences without duplication. Expired trivia stays absent from current state. The resumed GM reuses required active values and preserves pending choices; missing required state remains a source gap rather than permission to reroll. Initial establishment itself writes no file and starts no automatic save. |
