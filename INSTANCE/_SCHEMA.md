...
| UPGRADE SETTING BRIEF | when absent, one complete explicitly accepted required `MODULES/<id>/SETTING_BRIEF.md`, through its candidate and published without changing campaign revisions | unsaved PLAY; an existing or malformed Setting Brief; inferred/new setting facts; every other MODULE path; CURRENT_SAVE, CAMPAIGN_CONTRACT, BEARING, ENGINE, INSTANCE, ARCHIVE, safety, or ledgers |
| MIGRATE V0.5 | when absent, one explicitly accepted required `MODULES/<id>/SETTING_BRIEF.md`; identity-only bound-empty BEARING; explicitly accepted revision-one CAMPAIGN_CONTRACT; legacy CURRENT_SAVE preserved and upgraded as a new checkpoint, published last | unsaved PLAY; inferred goals/mandate/bearing; every other MODULE write; ENGINE, ARCHIVE, PC/person/system history; legacy fact loss or reinterpretation |
...
## v0.6/v0.6.1 Setting Brief compatibility upgrade

Installing v0.6.2 generic files over an already-bound v0.6 or v0.6.1 campaign may leave its otherwise valid module without the now-required compact Setting Brief. Ordinary boot, LOAD, CHECKPOINT, CLOSE, or silent normalization may not add it. Only an explicit **UPGRADE SETTING BRIEF** request may route to `ADMIN/ADD_SETTING_BRIEF.md` before the normal required-brief check.

The operator must attest the source version, a restorable byte-for-byte backup outside the active folder, and that CURRENT_SAVE includes every accepted PLAY. The existing bound save and accepted contract must otherwise validate and match. The fixed and candidate Setting Brief paths must both be absent; a malformed existing brief or candidate residue is a separate defect, not permission to overwrite.

ADMIN may then draft only a compact restatement of already-authoritative stable public module facts, show the complete body and its support, and require exact acceptance. It stages the accepted body as `SETTING_BRIEF.candidate.md`, validates it against the module contract, publishes it unchanged at the required fixed path, removes the candidate after byte comparison, and validates the complete tree. No new fact may be inferred from genre or filled for completeness.

This is the sole write: it does not revise CURRENT_SAVE, CAMPAIGN_CONTRACT, BEARING, any other MODULE record, ENGINE, INSTANCE, ARCHIVE, safety, or a ledger. It mints no revision and performs no PLAY. After success, use a fresh chat and the normal five-file technical boot.
...
The migration leaves the legacy CURRENT_SAVE authoritative while it stages a complete explicitly accepted revision-one Campaign Contract, an identity-only bound-empty Bearing, and a complete candidate save. v0.6.2 also requires the bound module's compact Setting Brief: preserve an existing valid brief byte-for-byte, or, when none exists, stage and require explicit acceptance of that one module addition from stable public facts and operator clarification. A malformed existing brief is repaired separately rather than overwritten during migration. The Contract and candidate save retain the existing `campaign_id` and `module`. The save preserves every legacy fact and allowed field, adds the four orientation fields from accepted records/operator clarification, assigns a new unique `save_id`, increments `save_rev` by one, names the old id as `save_parent`, uses `commit_kind: checkpoint`, and sets `archive_ref: none`. Never infer goals from conduct, bearing from repetition, a mandate from old POLICY tone, a causal item from compatibility, or setting facts from genre.

After all staged bodies validate, publish the accepted Setting Brief first when one was required, publish the bound-empty Bearing, publish CAMPAIGN_CONTRACT immediately before CURRENT_SAVE, and publish CURRENT_SAVE last as the commit point. Any partial write is fail-stop and requires restoration or exact completion from the external backup; it never authorizes PLAY. Migration does not rewrite or migrate any other MODULE content, ENGINE, ARCHIVE, the PC/person/system records, safety, or old seeds. A later optional REVIEW may create provisional Bearing only after the migrated checkpoint succeeds.
